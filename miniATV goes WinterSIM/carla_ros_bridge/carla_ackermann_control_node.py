...
from ackermann_msgs.msg import AckermannDriveStamped # CS: AckermannDrive changed to AckermannDriveStamped
...
class CarlaAckermannControl(object):
    ...
    def __init__(self):
        ...
        self.info.status.throttle_lower_border = 0. #CS: Originally 0.

        # control output
        self.info.output.throttle = 1. #CS: Originally: 0.
        ...
        self.control_subscriber = rospy.Subscriber(
            ...
            AckermannDriveStamped, self.ackermann_command_updated) # CS: AckermannDrive changed to AckermannDriveStamped
    ...
    def destroy(self):
        ...
        Terminate ROS subscription on AckermannDriveStamped commands.
    ...
    def ackermann_command_updated(self, ros_ackermann): # CS: changed to accomodate AckermannDriveStamped structure
        """
        Stores the ackermann drive message for the next controller calculation

        :param ros_ackermann: the current ackermann control input
        :type ros_ackermann: ackermann_msgs.AckermannDriveStamped
        :return:
        """
        self.lastAckermannMsgReceived = datetime.datetime.now()
        # set target values
        self.set_target_steering_angle(ros_ackermann.drive.steering_angle)
        self.set_target_speed(ros_ackermann.drive.speed)
        self.set_target_accel(ros_ackermann.drive.acceleration)
        self.set_target_jerk(ros_ackermann.drive.jerk)
    ...
    def control_stop_and_reverse(self):
        ...
        if self.info.current.speed_abs < standing_still_epsilon:
            ...
            if self.info.target.speed_abs < full_stop_epsilon:
                ...
                self.info.output.hand_brake = True
                self.info.output.brake = 1.0
                self.info.output.throttle = 1.0 #CS: Originally: 0.0
    ...
    def update_drive_vehicle_control_command(self):
        ...
        if self.info.status.accel_control_pedal_target > self.info.status.throttle_lower_border:
            ...
            # no control required
            self.info.output.brake = 0.0
            self.info.output.throttle = 0.0 #CS: Originally 0.0
        else:
        ...
        self.info.output.throttle = numpy.clip(
            self.info.output.throttle, 0., 1.) #CS: Originally: .throttle, 0., 1.)
