...
class World(object):
    ...
    def __init__(self, role_name, hud):
        ...
        #self.dt_miniATV_start_loc = self.geodetic_gnss_to_carla_location(66.4807052612, 25.7217578888, 78.8890151978) #lat, lon, alt 73.5652008057
        ...
        # Good camera perspective for test2
        ...
        #test3 side view: Transform(Location(x=3.335717, y=140.986176, z=78.637474), Rotation(pitch=-3.652519, yaw=105.446297, roll=-0.142578))
        ...
        # For test 3: Spawn reindeer in the test track for test 3
        #reindeer_bp = self.world.get_blueprint_library().filter('walker.pedestrian.0030')[0]
        #reindeer_bp = self.world.get_blueprint_library().filter('static.prop.gnome')[0]
        ...
        # Test1 Evaluation:
        ref_dict, gnss_dict = self.test1_to_carla_positions()
        #print(gnss_dict)
        #print(ref_dict)
        
    def test1_to_carla_positions(self):
        #Load csv files for the reference points and the gnss measurements from test1
        test1_averages = '/home/cs/Documents/Master_Thesis/Test_Data/Test1_Alignment/averages_gnss_test1.csv'
        ...
        gnss_dict = {}
        ref_dict = {}
        #test1_gnss_carla_pos = '/home/cs/Documents/Master_Thesis/Test_Data/Test1_Alignment/test1_averages_in_carla_location.csv'
        gnss_csv_rows = []
        gnss_csv_header = ['Ref Name', 'GNSS#', 'Ref&GNSS#', 'carla Location', 'x', 'y', 'z', 'dist_x', 'dist_y', 'dist_z']
        ref_csv_header = ['Ref Name', 'carla Location', 'x', 'y', 'z']
        ref_csv_rows = []
        # Put the relevant data into dicts for easier storage and comparison
        for ref in reference[1:]:
            avg_dict = {}
            print(ref[0])
            print("Ref Carla Position")
            ref_carla_location = carla.Location(self.geodetic_gnss_to_carla_location(float(ref[4]), float(ref[5]), float(ref[3])))
            print(ref_carla_location)
            print(type(ref[5]))
            print(ref[5])
            print([float(ref[5])])
            print([float(ref[5])])
            ...
            for avg in averages[1:]:
                if ref[0] in avg[0]:
                    print(avg[0])
                    # compare lat long and alt!!!
                    # get rid of the numbers in the name
                    #print(type(avg[0]))
                    avg_key = str(avg[0])[:-24]
                    #print(avg_key)
                    gnss_key = avg_key[-5:]
                    #print(gnss_key)
                    #TODO: Convert to carla positon before!
                    avg_key_lat = avg[1]
                    avg_key_lon = avg[2]
                    avg_key_alt = avg[3]
                    #print(str(gnss_key) + " carla position")
                    avg_carla_location = carla.Location(self.geodetic_gnss_to_carla_location(float(avg_key_lat), float(avg_key_lon), float(avg_key_alt)))
                    dist_x, dist_y, dist_z =self.dist_btw_carla_locs(ref_carla_location, avg_carla_location)
                    print(avg_carla_location().z)
                    #print(avg_key_lat)
                    #print(avg_key_lon)
                    #print(avg_key_alt)
                    gnss_csv_rows.append([ref[0], gnss_key, avg_key, avg_carla_location, avg_carla_location.x, avg_carla_location.y, avg_carla_location.z, dist_x, dist_y, dist_z])
                    avg_dict.update({gnss_key:{avg_carla_location}})
            
            gnss_dict.update({ref[0]:avg_dict})
        
        test1_ref_carla_pos = '/home/cs/Documents/Master_Thesis/Test_Data/Test1_Alignment/reference_points_in_carla_location.csv'
        test1_gnss_carla_pos = '/home/cs/Documents/Master_Thesis/Test_Data/Test1_Alignment/test1_averages_in_carla_location.csv'
        # write the carla positions to a csv file (in case of evaluation outside of carla)
        with open(test1_ref_carla_pos, 'w') as reffile:
            ref_writer = csv.writer(reffile) # creating a csv writer object
            ref_writer.writerow(ref_csv_header)
            ref_writer.writerows(ref_csv_rows)
            reffile.close()
        with open(test1_gnss_carla_pos, 'w') as gnssfile:
            gnss_writer = csv.writer(gnssfile) # creating a csv writer object
            gnss_writer.writerow(gnss_csv_header)
            gnss_writer.writerows(gnss_csv_rows)
            gnssfile.close()

        #print(gnss_dict)
        #print(ref_dict)
        return ref_dict, gnss_dict
